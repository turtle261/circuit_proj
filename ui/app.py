...
def extract_arduino_code_from_crew_result(crew_result):
    """Extract Arduino code from CrewAI results using multiple patterns."""
    import re
    
    result_str = str(crew_result)
    
    # Multiple patterns to find Arduino code
    patterns = [
        r'```(?:cpp|c\+\+|arduino)?\s*(.*?)```',
        r'```\s*(#include.*?)\s*```',
        r'(#include.*?void\s+loop\s*\(\s*\)\s*\{.*?\})',
        r'(void\s+setup\s*\(\s*\)\s*\{.*?void\s+loop\s*\(\s*\)\s*\{.*?\})',
        r'main_code["\']:\s*["\']([^"\']*)["\']',
        r'Arduino Code[:\s]*\n(.*?)(?:\n\n|\Z)',
    ]
    ...
    for pattern in patterns:
        ...
        for match in matches:
            code = match.strip()
            # Check if it looks like Arduino code
            if any(keyword in code.lower() for keyword in ['setup()', 'loop()', '#include', 'void setup', 'void loop']):
                return code
    
    # If no code found, generate basic template based on circuit type
    return generate_default_arduino_code(crew_result)
...
void loop() {
  ...
  delay(100);
    ...
    elif 'sensor' in result_str:
        return '''// Sensor Interface Circuit
// Generated by Circuit AI
...
def run_design_process(user_input: str):
    """Run the complete circuit design process with real-time updates using CrewAI."""
    try:
        ...
        try:
            ...
            circuit_type = determine_circuit_type(user_input)
            simulation_results = {"status": "AI-generated", "type": circuit_type}
            schematic_results = {"status": "AI-generated", "type": circuit_type}
            plot_data = None  # AI doesn't generate plots yet
        ...
        # Initialize variables for both CrewAI success and fallback cases
